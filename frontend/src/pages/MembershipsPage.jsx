...
export default function MembershipsPage() {
  ...
  const pay = async () => {
    ...
    setProcessing(true);
    try {
      if (method === "upi" && !upiId.includes("@") && !config.razorpay_enabled) throw new Error("Enter a valid UPI ID (e.g. name@bank)");
      ...
      // Create order (handles both real and mock paths)
      ...
      {selected && (
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" data-testid="payment-modal">
          <div className="card-dark p-8 max-w-lg w-full relative">
            ...
            <div className="mt-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[{v:"upi",l:"UPI"},{v:"debit_card",l:"Debit"},{v:"credit_card",l:"Credit"}].map(m => (
                  <button key={m.v} onClick={() => setMethod(m.v)} className={`p-3 border text-sm uppercase tracking-widest font-bold transition-colors ${method===m.v?"border-orange text-orange":"border-white/10 text-white/70"}`} data-testid={`pay-method-${m.v}`}>
                    {m.l}
                  </button>
                ))}
              </div>
            ...
            {config.razorpay_enabled ? (
              <p className="text-xs text-white/60 mt-5">You'll be redirected to Razorpay's secure checkout. Use the selected method there.</p>
            ) : method === "upi" ? (
              <div className="mt-5">
